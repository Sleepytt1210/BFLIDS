...
const main = async () => {
    ...
    router.post('/transactions/checkpoint/create', async (req: Request, resp: Response) => {
        try {
            ...
            console.log(
                `Creating new checkpoint with args ${JSON.stringify(checkpointData)} from client ${clientName}`
            );

            const { gateway, network, contract } = await setupConnection(
                clientName,
                channelName,
                chaincodeName,
                contractName
            ...
            await createCheckpoint(
                ...
                checkpointData.hash,
                checkpointData.url,
                checkpointData.algorithm,
                checkpointData.cAccuracy,
                checkpointData.loss,
                checkpointData.round,
                checkpointData.fedSession
            );
    ...
    async function setupConnection(
        ...
        contractName: string
    ): Promise<{ gateway: Gateway; network: Network; contract: Contract }> {
